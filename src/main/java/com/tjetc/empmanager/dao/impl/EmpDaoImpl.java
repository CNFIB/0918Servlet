...
public class EmpDaoImpl implements EmpDao {
    ...
    public int insert(Emp emp) {
        ...
        PreparedStatement pstmt = DButil.getPstmt(conn,"insert into emp(empno,ename,job,mgr,hiredate,sal,comm,deptno)"+"values(?,?,?,?,?,?,?)",emp.getEmpno(),
                emp.getEname(),emp.getJob(),emp.getMgr(),emp.getHiredate(),emp.getSal(),emp.getComm(),emp.getDept().getDeptno());
    ...
    public Emp selectByIdAndName(int empno, String ename) {
        Connection conn = DButil.getConnection();
        PreparedStatement pstmt = DButil.getPstmt(conn,"select * from emp e,dept d where empno=? and ename=?"+"and e.deptno = d.deptno",
                empno,ename);
    ...
    public List<Emp> selectManager() {
        Connection conn = DButil.getConnection();
        PreparedStatement pstmt = DButil.getPstmt(conn,"select * from emp e,dept d where"+"empno in(select distinct mgr from emp where mgr is not null)"+
